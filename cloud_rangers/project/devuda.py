    ...
    def fetch_from_openfoodfacts(self, barcode: str) -> dict | None:
        ...
        try:
            ...
            result = {
                ...
                "brand":      p.get("brands", ""),
                "category":   p.get("categories_tags", [""])[0].replace("en:", "").replace("-", " ").title() if p.get("categories_tags") else p.get("categories", ""),
...
class AdditiveService:
    ...
    def analyze_additives(self, additive_codes: list[str]) -> list[dict]:
        ...
        name_col = next((c for c in db.columns if "name" in c.lower() and "additive" in c.lower()), None) or \
                   next((c for c in db.columns if "name" in c.lower()), None)
...
class ScoringEngine:
    ...
    def calculate_concern_score(
        ...
        # Product banned (penalty 20 per country, capped 20)
        if ban_hits:
    ...
    def personalized_warnings(
        ...
        sat_fat = n.get("saturated-fat_100g", 0) or 0
        fat     = n.get("fat_100g",    0) or 0
        high_additives = [a for a in additive_results if (a.get("risk_level") or "").lower() == "high"]
        ...
        if user_type == "Child":
            if high_additives:
                names = ", ".join(a["name"] for a in high_additives[:3] if a["name"] != "Unknown")
...
"""
    ...
    def format_report_html(self, raw_text: str) -> str:
        """Convert numbered sections to styled HTML."""
        sections = {
        ...
        for line in lines:
            ...
            for key, label in sections.items():
                if stripped.startswith(key) or (stripped[:3].strip().rstrip(".").isdigit() and label.lower() in stripped.lower()):
                    html += f"<h3>{label}</h3>"
...
def _grade_color(grade: str) -> str:
    return {"A":"#238636","B":"#85bb2f","C":"#9e6a03","D":"#bd561d","E":"#da3633"}.get(grade.upper(), "#30363d")
...
def _process_barcode(
    ...
    """Fetch and enrich a product. Returns enriched dict or None."""
...
def main():
    ...
    # Session state
    for key, val in [("product", None), ("history", [])]:
    ...
    with st.sidebar:
        ...
        if st.session_state.history:
            for idx, h in enumerate(st.session_state.history[:6]):
                ...
                label, cls, color = scorer.determine_risk_label(score)
                if st.button(
    ...
    if search_clicked and barcode_input.strip():
        ...
        if product:
            ...
            # Add to history (deduplicate by barcode)
    ...
    if st.session_state.product:
            ...
            with col_info:
                ...
                for m_label, m_val, m_unit in snap:
                    metric_html += (
                        ...
                        f'<div class="m-value">{float(m_val):.1f}</div>'
        ...
        with t_bans:
            ...
            if bans:
                ...
                if not os.path.exists(BANNED_PRODUCTS_CSV):
                    st.caption(f"Note: Banned products CSV not found at `{BANNED_PRODUCTS_CSV}`. Set BANNED_CSV env var to load your file.")
        ...
        <div style="text-align:center;padding:4rem 0;color:#6e7681">
          ...
          <p style="font-size:1rem;color:#8b949e">Enter a product barcode above to begin analysis</p>
