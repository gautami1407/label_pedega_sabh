...
class FoodLabelAnalyzerApp:
    def __init__(self):
        ...
        self.client = None  # Initialize for new SDK
    ...
    def _configure_gemini(self):
        ...
        try:
            # New official SDK initialization
            self.client = genai.Client(api_key=api_key)
            self.logger.info(f"Gemini Client initialized successfully with model: {self.config.GEMINI_MODEL}")
    ...
    def analyze_food_image(self, image):
        try:
            ...
            try:
                # Use standardized model name from config
                ...
                if "invalid" in str(api_error).lower() or "api_key" in str(api_error).lower():
                    return "API Error: Invalid API key. Please check your settings."
                return f"AI Generation Error: {str(api_error)}. Please try again."
    ...
    def get_specific_analysis(self, question):
        try:
            # Use self.client which is already initialized in __init__
            if not hasattr(self, 'client') or self.client is None:
                self._configure_gemini()
            
            # Update model reference for the new SDK
            model_name = self.config.GEMINI_MODEL
